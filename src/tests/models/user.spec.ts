import { BaseUser, User, UserStore } from '../../models/user';
...
describe('Test User Model', () => {
  ...
  it('check if can delete user', async () => {
    const ck: User = await userStore.create(testUser);
  ...
  it('check if can update user', async () => {
    ...
    const { firstname, lastname } = await userStore.update( updateUser.firstname,
      updateUser.lastname,ck.id);
